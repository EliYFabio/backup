...
input double atr_sl=2;
input double atr_p=14;
input double gridInterval=40;
input int MagicNumber=1111;
input int    period=50;
input int   grid_layers=3;
...
void OnTick()
  {
//---
   if(isFirstFriday() && isTimeBetween(13,30,20,30))
   {
...
void grid(int numLayers)
   ...
   float atr=iATR(Symbol(),PERIOD_D1,atr_p,0);
   for (i = 1; i <= numLayers-1; i++) {
     ...
     OrderSend(Symbol(), OP_BUYSTOP, lot_size*pow(multiplier,i), upperGrid, 3, upperGrid - atr*atr_sl,0, "Grid Sell", MagicNumber, 0, Red);
     // Place buy limit order at lower grid level
     OrderSend(Symbol(), OP_SELLSTOP, lot_size*pow(multiplier,i), lowerGrid, 3, lowerGrid + atr*atr_sl, 0, "Grid Buy", MagicNumber, 0, Blue);
   ...
   OrderSend(Symbol(), OP_BUYSTOP, lot_size*pow(multiplier,i), upperGrid, 3, upperGrid - atr*atr_sl, atr*atr_tp, "Grid Sell", MagicNumber, 0, Red);
     // Place buy limit order at lower grid level
   OrderSend(Symbol(), OP_SELLSTOP, lot_size*pow(multiplier,i), lowerGrid, 3, lowerGrid + atr*atr_sl, lowerGrid - atr*atr_tp, "Grid Buy", MagicNumber, 0, Blue);
